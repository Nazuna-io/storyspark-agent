...
def mock_config():
    ...
    return {
        "agent": {"schedule_interval_minutes": 60, "max_sparks_per_cycle": 3, "run_immediately_on_start": True},
        "trend_detection": {"history_window_days": 7},
        "logging": {"output_file": "test_sparks.md"},
    }
...
def sample_history():
    ...
    return [{"title": "Old Item", "timestamp": datetime.now(timezone.utc) - timedelta(days=2), "source": "test_source"}]
...
def sample_new_items():
    """Sample new items."""
    return [
        {"title": "New Item 1", "timestamp": datetime.now(timezone.utc), "source": "test_source"},
        {"title": "New Item 2", "timestamp": datetime.now(timezone.utc), "source": "test_source"},
    ]
...
def sample_timestamps():
    ...
    return {"test_source": datetime.now(timezone.utc) - timedelta(days=1)}
...
class TestDatetimeSerialization:
    ...
    def test_datetime_parser(self):
        ...
        data = {"timestamp": "2024-01-01T12:00:00+00:00", "other_field": "test"}
    ...
    def test_datetime_parser_invalid_format(self):
        ...
        data = {"timestamp": "invalid-datetime", "other_field": "test"}
        result = _datetime_parser(data)
...
class TestRunAgentCycle:
    ...
    def test_run_cycle_no_new_items(
        self, mock_configure, mock_generate, mock_detect, mock_get_items, mock_config, sample_history, sample_timestamps
    ):
    ...
    @patch("src.main.generate_story_seed")
    @patch("src.main.configure_genai")
    def test_run_cycle_with_sparks(
        self,
        mock_configure,
        mock_generate,
        mock_detect,
        ...
        mock_get_items.return_value = (sample_new_items, sample_timestamps)
        mock_sparks = [{"keyword": "test1", "source_name": "source1"}, {"keyword": "test2", "source_name": "source2"}]
    ...
    @patch("src.main.generate_story_seed")
    @patch("src.main.configure_genai")
    def test_run_cycle_max_sparks_limit(
        self,
        mock_configure,
        mock_generate,
        mock_detect,
    ...
    @patch("src.main.detect_sparks")
    @patch("src.main.configure_genai")
    def test_run_cycle_history_window(self, mock_configure, mock_detect, mock_get_items, mock_config):
        ...
        new_items = [{"title": "New Item", "timestamp": datetime.now(timezone.utc), "source": "test_source"}]
    ...
    @patch("src.main.generate_story_seed")
    @patch("src.main.configure_genai")
    def test_run_cycle_api_configuration_failure(
        self,
        mock_configure,
        mock_generate,
        mock_detect,
...
class TestScheduledJob:
    ...
    def test_scheduled_job_success(
        self, mock_save_markdown, mock_save_json, mock_save_state, mock_run_cycle, mock_config
    ):
        ...
        def scheduled_job():
            updated_history, updated_timestamps, new_seeds = mock_run_cycle(
                mock_config, state_container["history"], state_container["timestamps"]
            )
