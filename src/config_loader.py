...
def load_config(path: str = "config.yaml") -> Dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(config['trend_detection'], dict): raise ValueError("'trend_detection' must be a dictionary.")
        if not isinstance(config['generation'], dict): raise ValueError("'generation' must be a dictionary.")
        if not isinstance(config['logging'], dict): raise ValueError("'logging' must be a dictionary.")
        if not isinstance(config['agent'], dict): raise ValueError("'agent' must be a dictionary.")
        ...
        if not config['agent'].get('schedule_interval_minutes') or not isinstance(config['agent']['schedule_interval_minutes'], int):
             raise ValueError("Missing or invalid 'agent.schedule_interval_minutes' (integer) in configuration.")
        if not config['generation'].get('prompt_template') or not isinstance(config['generation']['prompt_template'], str):
             raise ValueError("Missing or invalid 'generation.prompt_template' (string) in configuration.")
        if not config['trend_detection'].get('history_window_days') or not isinstance(config['trend_detection']['history_window_days'], int):
