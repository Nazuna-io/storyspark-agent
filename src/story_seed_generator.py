...
def _parse_gemini_response(text: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not what_if_questions:
            logger.warning(
                f"Could not parse 'What If Questions' from LLM response.\n" f"Response snippet:\n{text[:500]}..."
            )
            # Decide if we allow seeds without questions/keywords. For now, let's require them.
            return None
        if not thematic_keywords:
            logger.warning(
                f"Could not parse 'Thematic Keywords' from LLM response.\n" f"Response snippet:\n{text[:500]}..."
            )
            return None
...
def generate_story_seed(spark: Dict[str, Any], config: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    for attempt in range(api_max_retries + 1):
        try:
            ...
            if response_text:
                logger.debug(
                    f"Raw Gemini response for spark '{spark_keyword}':\n"
                ...
                if parsed_content:
                    ...
                    logger.error(
                        f"Failed to parse the Gemini response for spark '{spark_keyword}'. "
                ...
                logger.warning(
                    f"Gemini API returned no text content for spark '{spark_keyword}' "
                ...
                if finish_reason == "STOP" and not block_reason:
                    ...
                    logger.warning(
                        "Generation stopped due to max tokens. " "Consider revising prompt or model settings. No retry."
                    )
                ...
                if should_retry and attempt < api_max_retries:
                    ...
                    logger.error(
                        f"Gemini returned no text content after {attempt + 1} attempts " f"or retry not warranted."
                    )
            ...
            logger.error(
                f"Error during Gemini API call "
                f"(attempt {attempt + 1}/{api_max_retries + 1}) "
                f"for spark '{spark_keyword}': {e}",
                exc_info=True,
            )
            if attempt < api_max_retries:
                delay = api_retry_delay * (attempt + 1)
                logger.info(
                    f"Retrying Gemini request after error "
                    f"(attempt {attempt + 2}/{api_max_retries + 1}) "
                    f"after {delay} seconds..."
                )
                time.sleep(delay)
